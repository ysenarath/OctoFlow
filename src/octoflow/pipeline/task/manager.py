...
from octoflow.pipeline.task.task import Task
...
class TaskManager:
    def __init__(
        ...
        for i, task in enumerate(tasks):
            hash = task.hash()
            hash_filepath = (
                self.path / ".octoflow" / "tasks" / f"task-{i}" / "hash.txt"
            )
    ...
    def iter_tasks(self) -> Iterable[Tuple[int, Task, List[Run]]]:
        """
    ...
    def run(self, index: int) -> None:
        ...
        hash_filepath = (
            self.path / ".octoflow" / "tasks" / f"task-{index}" / "hash.txt"
        )
        hash = hash_filepath.read_text().strip()
        expr = Experiment(self.path)
